...
############################################################
########## Gasoline (per capita kg) data analysis ##########
############################################################
...
gasoline_m0_pre <- glm(benzina_pro_capite ~ 
                         Month_fct + 
                         Year + 
                         Tourists_stays_pc + 
                         hdd + 
...
accuracy_results <- accuracy_for(model = gasoline_m0_pre, model_name = "m0",
                                 y_name = "benzina_pro_capite",
                                 test_set = pretest_imp, accuracy_results)$accuracy_results

##### M1: GAM with Gamma response
gasoline_m1_pre <- gam(benzina_pro_capite ~ 
                         s(Month, bs="cp", k = 12) +
                       ...
                       family = Gamma(link=log),
                       data = pretrain_imp)
accuracy_results <- accuracy_for(model = gasoline_m1_pre, model_name = "m1",
                                 y_name = "benzina_pro_capite",
                                 test_set = pretest_imp, accuracy_results)$accuracy_results

##### M2: Optimized GAM (w.r.t. spatial smoothing) with Gamma response
# gam.check(gasoline_m1_pre)
if (Optim_GAM_k == T) {
  ...
  save(k_space,gam_k_space,perf_mat_gams,accuracy_gams, file = "Gasoline_Output_optimK.RData")
} else {
  load("Gasoline_Output_optimK.RData")
}
...
p_optGAMs1 <- perf_mat_gams %>%
  ...
  labs(title = "Gasoline: in-sample optimization of GAMs (fit and complexity trade-off)",
       subtitle = "Values represent the metrics obtained using a certain number of nodes",
       x = "Knots", y = "Metric")
ggpubr::ggexport(plotlist = list(p_optGAMs1), filename = "Gasoline_ISMetricsGAMs.png", width = 1500, height = 900, res = 100)
...
ggpubr::ggexport(plotlist = list(p_optGAMs2), filename = "Gasoline_ISGainGAMs.png", width = 1500, height = 900, res = 100)
...
p_optGAMs3 <- accuracy_gams_long %>%
  ...
  geom_point(data = accuracy_gams_long %>%
             ...
             size=4, colour="orange") +
  facet_wrap(~ Index, scales = "free") + 
  labs(title = "Gasoline: out-of-sample optimization of GAMs (fit and complexity trade-off)",
       subtitle = "Values represent the metrics obtained using a certain number of nodes",
       x = "Knots", y = "Metric")
ggpubr::ggexport(plotlist = list(p_optGAMs3), filename = "Gasoline_OOSoptimGAMs.png", width = 1500, height = 900, res = 100)

# Optimal number of nodes
Optimal_k <- 92
gasoline_m2_pre <- gam_k_space[[which(k_space == Optimal_k)]]
accuracy_results <- accuracy_for(model = gasoline_m2_pre, model_name = "m2",
                                 y_name = "benzina_pro_capite",
                                 test_set = pretest_imp, accuracy_results)$accuracy_results
gam.check(gasoline_m2_pre)

##### M3: GAM (Gamma response) with space-time interaction (monthly seasonality and spatial component)
gasoline_m3_pre <- gam(benzina_pro_capite ~ 
                         s(Month, bs="cp", k = 12) +
                         s(Year, bs="bs",m=c(3,2)) +
                         s(Tourists_stays_pc,bs = "bs",m=c(3,2)) +
                         s(cdd, bs="bs",m=c(3,2)) +
                       ...
                       family = Gamma(link=log),
                       data=pretrain_imp)
accuracy_results <- accuracy_for(model = gasoline_m3_pre, model_name = "m3",
                                 y_name = "benzina_pro_capite",
                                 test_set = pretest_imp, accuracy_results)$accuracy_results
...
save(gasoline_m0_pre,gasoline_m1_pre,gasoline_m2_pre,gasoline_m3_pre,
     pretrain_imp,pretest_imp,accuracy_results,perf_metr,
     file = "Gasoline_ForecastAssessment_Output.RData")
...
p_comb <- annotate_figure(p = p_comb,
                          top = text_grob("Gasoline: out-of-sample prediction metrics. \n Train: 2015-2018 & test: 2019", size = 12,face = "bold"),
                          bottom = text_grob("Note: RMSE and MAE are measured in kg/capita. MAPE, WAPE, SWAPE and R2 are percentages.",
                                             size = 10))
ggexport(p_comb,width = 1800, height = 1200, res = 150, filename = "Gasoline_OOSForecastMetrics.png")
...
for (i in 1:h_max){
  ...
  prev_m0 <- exp(predict(gasoline_m0_pre, newdata = pretest_imp))
  resid_m0 <- pretest_imp$benzina_pro_capite - prev_m0
  
  prev_m1 <- exp(predict(gasoline_m1_pre, newdata = pretest_imp))
  resid_m1 <- pretest_imp$benzina_pro_capite - prev_m1
  
  prev_m2 <- exp(predict(gasoline_m2_pre, newdata = pretest_imp))
  resid_m2 <- pretest_imp$benzina_pro_capite - prev_m2
  
  prev_m3 <- exp(predict(gasoline_m3_pre, newdata = pretest_imp))
  resid_m3 <- pretest_imp$benzina_pro_capite - prev_m3
...
p_dm <- dm_gam %>%
  ...
  labs(y = "P-value", x = "Forecasting horizon (months)",
       title = "Gasoline: pairwise Diebold-Mariano test with right-side alternative hypothesis",
...
ggexport(p_dm,width = 1800, height = 1200, res = 150, filename = "Gasoline_DieboldMariano.png")
...
gasoline_m2 <- gam(benzina_pro_capite ~ 
                     s(Month, bs="cp", k = 12) +
                     s(Year, bs="bs",m=c(3,2)) +
                     s(Tourists_stays_pc,bs = "bs",m=c(3,2)) +
                     s(cdd, bs="bs",m=c(3,2)) +
...
gasoline_m3 <- gam(benzina_pro_capite ~ 
                     s(Month, bs="cp", k = 12) +
                     s(Year, bs="bs",m=c(3,2)) +
                     s(Tourists_stays_pc,bs = "bs",m=c(3,2)) +
                     s(cdd, bs="bs",m=c(3,2)) +
...
##### Models coefficients
texreg(l = list(gasoline_m0,gasoline_m1,gasoline_m2,gasoline_m3),
...
save(gasoline_m0,gasoline_m1,gasoline_m2,gasoline_m3,
     file = "Gasoline_COVIDAssessment_Output.RData")
...
train_imp <- train_imp %>%
  mutate(pred_m0 = exp(predict(gasoline_m0, newdata=train_imp)),
         ...
         pred_m3 = exp(predict.gam(gasoline_m3, newdata=train_imp)),
         pred_err_m3 = benzina_pro_capite - pred_m3)
...
test <- test %>%
  mutate(pred_m0 = exp(predict(gasoline_m0, newdata=test)),
         ...
         pred_err_m2 = benzina_pro_capite - pred_m2,
         pred_m3 = exp(predict.gam(gasoline_m3, newdata=test)),
         pred_err_m3 = benzina_pro_capite - pred_m3)
...
sm <- smooth_estimates(gasoline_m3, smooth = "ti(long,lat,Month)", data = grd_ita)
sm_red <- sm %>%
...
p3a <- sm_red %>%
  ...
  labs(x = "Longitude", y = "Latitude", title = "Gasoline: estimated smooth space-time interaction (M3)") + 
  scale_fill_viridis(option="magma", name = "Smooth effect") + 
  scale_x_continuous(labels = function(x) paste0(x, '\u00B0', "E")) +
  scale_y_continuous(labels = function(x) paste0(x, '\u00B0', "N"))
ggpubr::ggexport(p3a,width = 1800, height = 1200, res = 150, filename = "Gasoline_m3_interaction.png")
...
p3b <- sm_red %>%
  ...
  geom_sf(data = st_crop(shape,xmin=6.5, xmax=19, ymin=36.5, ymax=47.2), col = "black", alpha =  0.05)+
  facet_wrap(~ Month_fct) + 
  labs(x = "Longitude", y = "Latitude", title = "Gasoline: estimated standard error for the smooth space-time interaction (M3)") + 
  scale_fill_viridis(option="magma", name = "SE") + 
  scale_x_continuous(labels = function(x) paste0(x, '\u00B0', "E")) +
  scale_y_continuous(labels = function(x) paste0(x, '\u00B0', "N"))
ggpubr::ggexport(p3b,width = 1800, height = 1200, res = 150, filename = "Gasoline_m3_interaction_SE.png")
...
grd_ita2 <- expand.grid(lat = seq(from = 36.5, to = 47.2, by = 0.05),
                        ...
                        SHAPE_AREA = 1)
##### Extract smoothed effect
sm2 <- smooth_estimates(gasoline_m3, smooth = "s(long,lat)", data = grd_ita2)
##### Map average smoothed effect
p4a <- sm2 %>%
  ggplot() +
  geom_tile(aes(y = lat, x = long, fill = est)) + 
  geom_contour(aes(x = long, y = lat, z = est), colour = "white", bins = 10) +
  geom_sf(data = st_crop(shape,xmin=6.5, xmax=19, ymin=36.5, ymax=47.2), col = "black", alpha =  0.05)+
  labs(x = "Longitude", y = "Latitude", title = "Gasoline: estimated smooth spatial effect (M3)") + 
  scale_fill_viridis(option="magma", name = "Smooth effect") + 
  scale_x_continuous(labels = function(x) paste0(x, '\u00B0', "E")) +
  scale_y_continuous(labels = function(x) paste0(x, '\u00B0', "N"))
ggpubr::ggexport(p4a,width = 1800, height = 1200, res = 150, filename = "Gasoline_m3_spatialsmooth.png")
##### Map SE of the smoothed effect
p4b <- sm2 %>%
  ggplot() +
  geom_tile(aes(y = lat, x = long, fill = se)) + 
  ...
  geom_sf(data = st_crop(shape,xmin=6.5, xmax=19, ymin=36.5, ymax=47.2), col = "black", alpha =  0.05)+
  facet_wrap(~ Month_fct) + 
  labs(x = "Longitude", y = "Latitude", title = "Gasoline: estimated standard error for the smooth spatial effect (M3)") + 
  scale_fill_viridis(option="magma", name = "SE") + 
  scale_x_continuous(labels = function(x) paste0(x, '\u00B0', "E")) +
  scale_y_continuous(labels = function(x) paste0(x, '\u00B0', "N"))
ggpubr::ggexport(p3b,width = 1800, height = 1200, res = 150, filename = "Gasoline_m3_spatialsmooth_SE.png")
...
p_corr <- corrmat %>%
  ...
  labs(title = "Gasoline: (dis)similarity measures for out-of-sample residuals by macro-regions")
ggexport(p_corr,width = 1800, height = 1200, res = 150, filename = "Gasoline_SimMeasures.png")